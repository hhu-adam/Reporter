...
def merge_translations(old_df: pandas.DataFrame, new_df: pandas.DataFrame) -> pandas.DataFrame:
    ...
    assert list(old_df.columns) == req_columns, f"Columns  of doc. DataFrame must be {req_columns} but were {old_df.columns}"
    assert list(new_df.columns) == req_columns, f"Columns  of new DataFrame must be {req_columns} but were {new_df.columns}"

    # Replace NaN values with zero.
    old_df = old_df.merge(new_df, how="outer", on=['country', 'game'], suffixes=['_old', '_new']).fillna(0)
...
def aggregate_over_interval(dfs: list[pandas.DataFrame]) -> pandas.DataFrame:
    assert len(dfs) > 0, "Provided list of DataFrames is empty. See if log files are available!"
    agg = dfs[0]
...
def get_location_cross_table_latex(agg: pandas.DataFrame) -> str:
    ...
    def sort_sum_cross_table(ctab: pandas.DataFrame) -> pandas.DataFrame:
        ...
        row_and_col_sorted = col_sorted.loc[row_sum.sort_values(ascending=False).index]
    ...
    latex_table = re.sub(r"begin{tabular}\{.*\}", "begin{tabular}{lrrrrrrrrrr}",latex_table)
    latex_table = re.sub(r"toprule", "toprule ~multicolumn{11}{c}{~large ~textbf{Timeframe: From " + start_date + " to " + end_date + "}} ~\ ~toprule", latex_table)
...
if __name__ == "__main__":
    ...
    f = open(relative_path(f"reports/report-{start_date}-{end_date}.tex"), mode='w')
    f.write(latex_table)
    f.close
